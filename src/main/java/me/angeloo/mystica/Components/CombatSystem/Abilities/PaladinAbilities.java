...
import me.angeloo.mystica.Components.CombatSystem.Abilities.Paladin.*;
...
public class PaladinAbilities {
    ...
    public int getAbilityCooldown(Player player, int abilityNumber){
        ...
        return switch (abilityNumber) {
            case 1 -> torahSword.getCooldown(player);
            case 2 -> divineGuidance.getCooldown(player);
            case 3 -> reigningSword.getCooldown(player);
            case 4 -> covenantSword.getReadyIn(player);
            case 5 -> orderShield.getCooldown(player);
            case 6 -> gloryOfPaladins.getCooldown(player);
            case 7 -> duranceOfTruth.getCooldown(player);
            case 8 -> judgement.getReadyIn(player);
            default -> 0;
        };
    ...
    public int getUltimateCooldown(Player player){
        ...
        return switch (subclass) {
            case Templar -> sanctityShield.getSkillCooldown();
            case Divine -> representative.getSkillCooldown();
            case Dawn -> lightWell.getSkillCooldown();
            default -> 0;
        };
    ...
    public int getPlayerUltimateCooldown(Player player){
        ...
        return switch (subclass) {
            case Templar -> sanctityShield.getPlayerCooldown(player);
            case Divine -> representative.getPlayerCooldown(player);
            case Dawn -> lightWell.getPlayerCooldown(player);
            default -> 0;
        };

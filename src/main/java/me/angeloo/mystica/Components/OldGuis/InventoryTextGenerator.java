package me.angeloo.mystica.Components.OldGuis;
...
public class InventoryTextGenerator {
    ...
    public String getEquipmentDifferenceText(Map<Object, Integer> deltas){

        StringBuilder builder = new StringBuilder();

        List<String> strings = new ArrayList<>();

        for(Map.Entry<Object, Integer> entry : deltas.entrySet()){

            //check if key is an integer only, then display "skill #"

            boolean negative = entry.getValue() < 0;

            if(negative){
                strings.add(entry.getKey().toString().replaceAll("_", " ") + " -" + Math.abs(entry.getValue()));
            }
            else{
                strings.add(entry.getKey().toString().replaceAll("_", " ") + " +" + Math.abs(entry.getValue()));
            }
        ...
        //Bukkit.getLogger().info(String.valueOf(strings));
        ...
        for(String s : strings){
            ...
            String activeColor = ChatColor.of(Color.RED).toString();

            if(s.contains("+")){
                activeColor = ChatColor.of(Color.GREEN).toString();
            }

            builder.append(activeColor);

            for(char c : s.toCharArray()){
                builder.append(getCharacter(c, index));
            }

            for(char c : s.toCharArray()){
                builder.append(CHAR_NEGATIVE_SPACE.getOrDefault(c,""));
            }

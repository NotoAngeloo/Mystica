...
public class ProfileManager {
        ...
        for(UUID uuid : playerProfiles.keySet()) {
            ...
            for(MysticaItem equipment : playerEquipment.getEquipment()){
                ...
                if(equipment == null){
                    ...
                    switch (eqSlot) {
                        case 0 -> {
                            config.set(id + ".equipment.WEAPON", null);
                        }
                        case 1 -> {
                            config.set(id + ".equipment.HEAD", null);
                        }
                        case 2 -> {
                            config.set(id + ".equipment.CHEST", null);
                        }
                        case 3 -> {
                            config.set(id + ".equipment.LEGS", null);
                        }
                        case 4 -> {
                            config.set(id + ".equipment.BOOTS", null);
                        }
                    }
    ...
    public Profile getNonPlayerProfileFromUUID(UUID uuid){
        return nonPlayerProfiles.get(uuid);
    }
    ...
    public Profile getAnyProfile(LivingEntity entity){
        ...
        if(nonPlayerProfiles.get(entity.getUniqueId()) == null && fakePlayerProfileMap.get(entity.getUniqueId()) == null){
            ...
            if(nonPlayerNameMap.containsKey(entity.getUniqueId())){
                ...
                creaturesAndCharactersManager.makeNonPlayerProfile(name, entity.getUniqueId());
            }
            else{
    ...
    public void addCompanion(Player player, UUID companion){
        ...
        /*if(cEntity instanceof LivingEntity livingEntity){

            Bukkit.getLogger().info(String.valueOf(MythicBukkit.inst().getAPIHelper().getMythicMobInstance(cEntity).getType()));


            Bukkit.getLogger().info("Attack: " + getAnyProfile(livingEntity).getStats().getAttack());
            Bukkit.getLogger().info("Health: " + getAnyProfile(livingEntity).getStats().getHealth());
            Bukkit.getLogger().info("Defense: " + getAnyProfile(livingEntity).getStats().getDefense());
            Bukkit.getLogger().info("Magic: " + getAnyProfile(livingEntity).getStats().getMagic_Defense());
            Bukkit.getLogger().info("Crit: " + getAnyProfile(livingEntity).getStats().getCrit());
        }*/
